...
type Props = {
  ...
  startContent: () => void; aiConfigured: boolean;
};
...
export function MonitoringWorkspace(props: Props) {
  ...
  return <>
    <div className="section-head"><div><div className="eyebrow">Curadoria de mercado</div><h1>Monitoramento</h1><p className="subtitle">Notícias reais persistidas, com classificação determinística e ações editoriais.</p></div><div className="inline-actions"><a className="secondary" href={exportHref}>Exportar CSV</a><button className="primary" disabled={!selectedIds.length} onClick={() => props.aiConfigured ? props.startContent() : props.notify("A fila editorial está preparada, mas a IA não está configurada nesta sprint.")}>Gerar conteúdo <span className="button-count">{selectedIds.length}</span></button></div></div>
    ...
    {!!filtered.length && <div className="selection-bar"><span>Ações em lote</span><div className="inline-actions wrap"><button onClick={() => props.toggleAll(props.selected.size === filtered.length ? [] : filtered.map((item) => item.id))}>{props.selected.size === filtered.length ? "Limpar seleção" : "Selecionar resultados"}</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("read", selectedIds)}>Marcar lidas</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("unread", selectedIds)}>Marcar não lidas</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("favorite", selectedIds)}>Favoritar</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("unfavorite", selectedIds)}>Remover favoritos</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("archive", selectedIds)}>Arquivar</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("restore", selectedIds)}>Restaurar</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("discard", selectedIds)}>Descartar</button><select className="compact-select" value={batchIcp} onChange={(event) => setBatchIcp(event.target.value)} aria-label="ICP para seleção"><option value="">Alterar ICP…</option>{ICP_CATALOG.map((icp) => <option key={icp.slug}>{icp.name}</option>)}</select><button disabled={!selectedIds.length || !batchIcp || props.busy} onClick={() => void mutate("setIcp", selectedIds, { primaryIcp: batchIcp })}>Aplicar ICP</button><input className="compact-input" value={batchTag} onChange={(event) => setBatchTag(event.target.value)} placeholder="Tag" aria-label="Tag para seleção" /><button disabled={!selectedIds.length || !batchTag.trim() || props.busy} onClick={() => { void mutate("addTag", selectedIds, { tag: batchTag }); setBatchTag(""); }}>Adicionar tag</button><button disabled={!selectedIds.length || props.busy} onClick={() => void mutate("selected", selectedIds)}>Fila editorial</button></div></div>}
