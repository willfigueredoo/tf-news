...
export function TFNewsApp({ userName, userEmail, initialUpdatedAt }: { userName: string; userEmail: string; initialUpdatedAt: string }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><button className="brand" type="button" onClick={() => chooseView("Visão Executiva")} aria-label="Ir para Visão Executiva"><img className="sidebar-logo" src="/brand/tf-news-icon.png" alt="" width="56" height="56" loading="eager" decoding="async" /></button><div className="nav-label">Newsroom</div><nav className="nav" aria-label="Navegação principal">{VIEWS.map((item) => <button key={item.name} className={`nav-button ${view === item.name ? "active" : ""}`} onClick={() => chooseView(item.name)} aria-label={item.name} data-tooltip={item.name === "Biblioteca" ? "Biblioteca Editorial" : item.name}><span className="nav-icon" aria-hidden="true">{item.icon}</span><span className="nav-text">{item.name}</span></button>)}</nav><div className="sidebar-foot"><div className="live-status"><span className="live-dot" /> Monitoramento operacional</div><div className="source-meta" style={{ marginTop: 8 }}>{sources.length} fonte(s) cadastrada(s)</div></div></aside>
    <main className="main"><header className="topbar"><div className="header-context"><button className="header-brand" type="button" onClick={() => chooseView("Visão Executiva")} aria-label="Ir para Visão Executiva"><img className="header-logo header-logo-light" src="/brand/tf-news-horizontal.png" alt="TF News" width="126" height="63" loading="eager" decoding="async" /><img className="header-logo header-logo-dark" src="/brand/tf-news-icon.png" alt="TF News" width="52" height="52" loading="eager" decoding="async" /></button><div><div className="crumb">Inteligência editorial</div><div className="page-name">{view}<span className="update-time">Atualizado às {lastUpdated}</span></div></div></div><div className="top-actions"><select className="global-select" value={globalIcp} onChange={(event) => setGlobalIcp(event.target.value)} aria-label="Filtrar todo o sistema por ICP"><option>Todos os ICPs</option>{ICP_CATALOG.map((icp) => <option key={icp.slug}>{icp.name}</option>)}</select><button className="theme-toggle" onClick={toggleTheme} aria-label="Alternar entre modo claro e escuro" title="Alternar tema"><span className="theme-icon-light" aria-hidden="true">☼</span><span className="theme-icon-dark" aria-hidden="true">◐</span></button><div className="user-chip" title={userEmail}><div className="avatar">{initials(userName)}</div><div className="user-copy"><strong>{userName}</strong><span>Editor</span></div></div></div></header>
