...
const STATUS_LABELS: Record<QueueStatus, string> = {
  new: "Nova", analysis: "Em análise", approved: "Aprovada", generating: "Gerando Kit",
...
export function EditorialQueue({ initialQueueId, onOpenKit, notify }: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? "Não foi possível carregar a Fila Editorial.");
  ...
  async function transition(item: EditorialQueueItem, target: QueueStatus) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? "A pauta não pôde ser atualizada.");
  ...
  return <>
    <div className="section-head">
      <div><div className="eyebrow">Organização editorial</div><h1>Fila Editorial</h1><p className="subtitle">Pautas independentes entre o Monitoramento e o acervo publicado.</p></div>
    ...
    <div className="editorial-origin-tabs" role="tablist" aria-label="Origem das pautas">
      <button type="button" role="tab" aria-selected={originFilter === "monitoring"} className={originFilter === "monitoring" ? "active" : ""} onClick={() => setOriginFilter("monitoring")}><strong>Monitoramento</strong><span>Notícias e sinais editoriais</span><i>{originCounts.monitoring}</i></button>
      <button type="button" role="tab" aria-selected={originFilter === "competitive"} className={originFilter === "competitive" ? "active" : ""} onClick={() => setOriginFilter("competitive")}><strong>Inteligência Competitiva</strong><span>Pautas inspiradas em concorrentes</span><i>{originCounts.competitive}</i></button>
    </div>
    {loading ? <div className="card empty"><strong>Atualizando a Fila Editorial…</strong></div> : !visible.length ? <div className="card empty"><strong>Nenhuma pauta neste estado</strong>{originFilter === "competitive" ? "Use Criar pauta inspirada em um artigo de concorrente para iniciar esta fila." : "Adicione notícias pelo Monitoramento para organizar a produção."}</div> : <div className="queue-list">
      {visible.map((item) => <article className={`card queue-item ${item.id === initialQueueId ? "highlight" : ""}`} key={item.id}>
        <div>
          <div className="content-meta">{isCompetitive(item) ? `Inspirada em ${item.competitorName ?? "concorrente"}` : item.sourceName ?? "Fonte monitorada"} · {item.primaryIcp ?? "ICP classificado"} · versão {item.version}</div>
          <h2>{item.competitorArticleTitle ?? item.title}</h2>
          {isCompetitive(item) && <div className="queue-origin-context"><span>Base factual: {item.sourceName ?? "fonte independente do Monitoramento"}</span>{item.competitorArticleUrl && <a href={item.competitorArticleUrl} target="_blank" rel="noopener noreferrer">Abrir referência ↗</a>}</div>}
          <div className="content-meta">Atualizada {formatDate(item.updatedAt)}{typeof item.relevanceScore === "number" ? ` · score ${item.relevanceScore}` : ""}</div>
        ...
        <div className="inline-actions wrap">
          ...
          {(["new", "analysis", "approved"] as QueueStatus[]).includes(item.status) && <button className="primary" disabled={busyId !== null} onClick={() => void generate(item)}>{busyId === item.id ? "Gerando…" : "Gerar Kit"}</button>}
