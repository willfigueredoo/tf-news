...
export async function GET() {
  try {
    ...
    const schema = await db.prepare("SELECT to_regclass('public.sources') AS sources, to_regclass('public.news_items') AS news_items, to_regclass('public.job_logs') AS job_logs").first<{ sources: string | null; news_items: string | null; job_logs: string | null }>();
