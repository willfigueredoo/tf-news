...
export const contentOpportunities = pgTable("content_opportunities", {
  ...
  priority: text("priority").notNull(),
  evergreenScore: integer("evergreen_score").notNull(),
  icpScore: integer("icp_score").notNull(),
  recurrenceScore: integer("recurrence_score").notNull(),
  contentGapScore: integer("content_gap_score").notNull(),
  competitorScore: integer("competitor_score").notNull(),
  googleSignalsScore: integer("google_signals_score"),
  scoreFactors: text("score_factors").notNull().default("{}"),
  primaryKeyword: text("primary_keyword").notNull(),
  ...
  generatedKitId: integer("generated_kit_id").references(() => editorialKits.id),
  publishedArticleId: integer("published_article_id").references(() => seoArticles.id),
  postponedReason: text("postponed_reason"),
  aiAnalysisId: integer("ai_analysis_id").references(() => seoAiAnalyses.id),
  lastError: text("last_error"),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [
  uniqueIndex("content_opportunities_topic_unique").on(table.normalizedTopic),
  uniqueIndex("content_opportunities_kit_unique").on(table.generatedKitId),
  index("content_opportunities_status_idx").on(table.status, table.strategicScore, table.updatedAt),
  index("content_opportunities_priority_idx").on(table.priority, table.strategicScore),
  index("content_opportunities_article_idx").on(table.publishedArticleId),
]);

export const contentOpportunitySources = pgTable("content_opportunity_sources", {
  id: serial("id").primaryKey(),
  opportunityId: integer("opportunity_id").notNull().references(() => contentOpportunities.id),
  sourceType: text("source_type").notNull(),
  newsItemId: integer("news_item_id").references(() => newsItems.id),
  competitorArticleId: integer("competitor_article_id").references(() => seoCompetitorArticles.id),
  seoArticleId: integer("seo_article_id").references(() => seoArticles.id),
  relevanceScore: integer("relevance_score").notNull().default(0),
  createdAt: text("created_at").notNull(),
}, (table) => [
  uniqueIndex("content_opportunity_sources_news_unique").on(table.opportunityId, table.newsItemId),
  uniqueIndex("content_opportunity_sources_competitor_unique").on(table.opportunityId, table.competitorArticleId),
  uniqueIndex("content_opportunity_sources_article_unique").on(table.opportunityId, table.seoArticleId),
  index("content_opportunity_sources_opportunity_idx").on(table.opportunityId, table.sourceType),
  index("content_opportunity_sources_news_idx").on(table.newsItemId),
  index("content_opportunity_sources_competitor_idx").on(table.competitorArticleId),
]);

export const contentOpportunityJobs = pgTable("content_opportunity_jobs", {
  id: serial("id").primaryKey(),
  jobKey: text("job_key").notNull().default("evergreen"),
  trigger: text("trigger").notNull(),
  status: text("status").notNull().default("queued"),
  cursor: integer("cursor").notNull().default(0),
  batchSize: integer("batch_size").notNull().default(3),
  ...
  ignored: integer("ignored").notNull().default(0),
  attempts: integer("attempts").notNull().default(0),
  candidateSnapshot: text("candidate_snapshot").notNull().default("[]"),
  lastError: text("last_error"),
  leaseOwner: text("lease_owner"),
  leaseExpiresAt: text("lease_expires_at"),
  nextRunAt: text("next_run_at").notNull(),
  ...
  index("content_opportunity_jobs_queue_idx").on(table.status, table.nextRunAt, table.createdAt),
  uniqueIndex("content_opportunity_jobs_active_unique")
    .on(table.jobKey)
    .where(sql`${table.status} in ('queued', 'processing', 'retry')`),
]);
